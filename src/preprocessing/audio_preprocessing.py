...
def preprocess_song(input_file_path, output_file_path, secs_per_img = 2):
    
    # NOTE: Changing secs_per_img will change the image dimensions, which will cause the decoder network to throw error. 
    # length of time (s) to ultimately reduce to a single point in latent space
    ...
    np.savez(output_file_path, F=F_crop, T=T, S_mag=S_mag_crop, dt=dt, img_width=img_width, img_height=img_height, time_resolution=time_resolution)

    vars = np.load(output_file_path)
    print(vars.files)
